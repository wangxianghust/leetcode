...
private:
    void combinationSum(vector<int> &candidates, int target, vector<int> &combination, vector<vector<int> > &ret, int begin){
        ...
        for(int i=begin; i<candidates.size() && target >= candidates[i]; ++i){
            ...
            combinationSum(candidates, target-candidates[i], combination, ret, i);
...
bool Solution::debug = false;


int main(){
    freopen("39test.txt", "r", stdin);
    int total;
    cin >> total;
    Solution Sol;
    for(int i=0; i<total; ++i){
        ...
        auto ret = Sol.combinationSum(can, target);
        Sol.print(ret);
    }
}
